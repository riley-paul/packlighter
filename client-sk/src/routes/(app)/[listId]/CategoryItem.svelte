...
>
	{#if list.show_packed}
		<Checkbox bind:checked={categoryItem.packed} onCheckedChange={saveCategoryItem} />
	{/if}
	...
	<div class="flex gap-0.5">
		<Input
			...
			class="h-auto min-w-0 border-none px-1 py-0.5 text-right shadow-none"
		/>
		<SpinBox bind:value={categoryItem.quantity} handleChange={saveCategoryItem} min={1} />
	</div>

	<input type="hidden" />
	<DeleteButton handleDelete={() => $deleteCategoryItem.mutate(categoryItem)} noConfirm />
	<DragHandle />
</form>
