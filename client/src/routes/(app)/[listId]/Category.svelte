<script lang="ts">
	...
	import { useCreateCategoryItem, useUpdateCategoryItemsOrder } from '@/hooks/useCategoryItem';
	import {
	...
	import { CATEGORY_NAME_CLASS, DRAGGABLE_CLASS, flipDurationMs } from '@/lib/constants';
	...
	const handleConsider = async (ev: CustomEvent<DndEvent<ExpandedCategoryItem>>) => {
		if ($isForeignItem) {
	...
	const handleFinalize = async (ev: CustomEvent<DndEvent<ExpandedCategoryItem>>) => {
