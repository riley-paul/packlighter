...
<div
	class={cn(
		DRAGGABLE_CLASS,
		'text-muted-foreground flex w-full items-center justify-between gap-2 pl-4 pr-2 text-base hover:border-l-4 hover:pl-3',
		!list.name && 'italic',
		$page.url.pathname.includes(list.id) && 'border-primary text-foreground border-l-4 pl-3'
	)}
>
	<a href={`/${list.id}`} class="flex h-8 flex-1 items-center">
